...
export default function Login() {
  ...
  const { user, isLoggedIn, register, loginWithEmail, loginWithGoogle, logout } = useUser();
    ...
    return (
      <div className="min-h-screen bg-background text-foreground">
          ...
          <div className="glass-effect border border-cyan-500/20 rounded-2xl overflow-hidden">
            <button onClick={() => navigate('/')}
              ...
              <div className="flex-1 text-left">
                ...
                <p className="text-xs text-gray-400">သင့်ပရိုဖိုင်ကို ကြည့်ရန်</p>
              </div>
          ...
          <div className="glass-effect border border-cyan-500/20 rounded-2xl overflow-hidden">
            <button className="w-full flex items-center gap-4 px-4 py-4">
              ...
              <div className="flex-1 text-left">
                ...
                <p className="text-xs text-gray-400">မူဝါဒကို ကြည့်ရှုပါ</p>
              </div>
              <ChevronRight size={18} className="text-gray-500" />
            </button>
          </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 glass-effect border-t border-cyan-500/20 px-6 py-3 flex justify-around">
          ...
          <button onClick={() => navigate('/profile')} className="flex flex-col items-center gap-1 text-gray-400 hover:text-cyan-400">
            <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      setError(result.error || 'Register မအောင်မြင်ပါ');
    }
    setLoading(false);
  };
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      setError(result.error || 'Login မအောင်မြင်ပါ');
    }
    setLoading(false);
  };
  ...
  const handleGoogle = async () => {
    ...
    if (result.success) {
      ...
      setError(result.error || 'Google login မအောင်မြင်ပါ');
    }
    setLoading(false);
  };
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              <div className="relative">
                ...
                <input type="text" placeholder="နာမည်" value={regName}
                  onChange={e => setRegName(e.target.value)} required disabled={loading}
              ...
              <div className="relative">
                ...
                <input type="email" placeholder="Gmail" value={regEmail}
                  onChange={e => setRegEmail(e.target.value)} required disabled={loading}
                  className="w-full pl-10 pr-4 py-3 bg-cyan-500/10 border border-cyan-500/30 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-cyan-400" />
              </div>
              <div className="relative">
                <Lock className="absolute left-3 top-3 w-4 h-4 text-gray-400" />
                <input type={showPassword ? 'text' : 'password'} placeholder="Password (အနည်းဆုံး ၆ လုံး)" value={regPassword}
                  onChange={e => setRegPassword(e.target.value)} required disabled={loading} minLength={6}
              ...
              <button type="button" onClick={handleGoogle} disabled={loading}
                className="w-full py-3 border border-cyan-500/30 rounded-lg text-white hover:bg-cyan-500/10 transition-all flex items-center justify-center gap-2 font-semibold disabled:opacity-50">
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <div className="relative">
                ...
                <input type="email" placeholder="Gmail" value={loginEmail}
                  onChange={e => setLoginEmail(e.target.value)} required disabled={loading}
                  className="w-full pl-10 pr-4 py-3 bg-cyan-500/10 border border-cyan-500/30 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-cyan-400" />
              </div>
              <div className="relative">
                <Lock className="absolute left-3 top-3 w-4 h-4 text-gray-400" />
                <input type={showPassword ? 'text' : 'password'} placeholder="Password" value={loginPassword}
                  onChange={e => setLoginPassword(e.target.value)} required disabled={loading}
                  className="w-full pl-10 pr-10 py-3 bg-cyan-500/10 border border-cyan-500/30 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-cyan-400" />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-3 text-gray-400">
                  {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                </button>
