...
function UserMenu() {
      ...
      <div className="relative">
        ...
        {dropdownOpen && (
          <div className="absolute right-0 mt-2 w-48 glass-hover border border-cyan-500/30 rounded-lg overflow-hidden z-50">
            <button onClick={() => { setLocation('/admin/dashboard'); setDropdownOpen(false); }}
              className="w-full px-4 py-3 text-left text-sm text-gray-300 hover:text-cyan-400 hover:bg-cyan-500/10 transition-all">
              Dashboard
            </button>
            <button onClick={async () => { await logoutAdmin(); setDropdownOpen(false); setLocation('/'); }}
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <button onClick={() => setLocation('/login')} className="btn-neon">
        Login
      </button>
    </div>
  );
      ...
      <div className="container mx-auto px-4 py-4">
        ...
        {isOpen && (
          <nav className="md:hidden mt-4 pb-4 border-t border-cyan-500/20 pt-4 flex flex-col gap-4">
            ...
            {isAdminLoggedIn ? (
              <>
                <button onClick={() => { setLocation('/admin/dashboard'); setIsOpen(false); }}
                  className="text-cyan-400 font-semibold text-left">Admin Dashboard</button>
                <button onClick={async () => { await logoutAdmin(); setIsOpen(false); setLocation('/'); }}
                  className="text-red-400 text-left flex items-center gap-2">
                  <LogOut size={16} /> Admin Sign Out
                </button>
              </>
            ) : isLoggedIn && user ? (
              <button onClick={() => { setLocation('/profile'); setIsOpen(false); }}
              ...
              <>
                <button onClick={() => { setLocation('/admin/login'); setIsOpen(false); }}
                  className="text-gray-400 hover:text-cyan-400 transition-colors text-left">Admin Login</button>
                <button onClick={() => { setLocation('/login'); setIsOpen(false); }}
                  className="btn-neon w-full">Login</button>
              </>
            )}
          </nav>
        )}
